// ignore_for_file: prefer_interpolation_to_compose_strings, await_only_futures, avoid_print, prefer_const_constructors, avoid_function_literals_in_foreach_calls, prefer_const_literals_to_create_immutables, unnecessary_new, sort_child_properties_last, prefer_is_empty
import 'dart:async';
...
class _MainScreenState extends State<MainScreen> with TickerProviderStateMixin
{
  ...
  void saveRideRequest()
  {
    ...
    rideStreamSubscription = rideRequestRef.onValue.listen((event) async {
      ...
      final eventData = event.snapshot.value as Map<String, dynamic>?;

      if (eventData == null) {
        return;
      }
      if(eventData["car_details"] != null){
        setState(() {
          carDetailsDriver = eventData["car_details"].toString();
        });
      }
      if(eventData["driver_name"] != null){
        setState(() {
          driverName = eventData["driver_name"].toString();
        });
      }
      if(eventData["driver_phone"] != null){
        setState(() {
          driverphone = eventData["driver_phone"].toString();
        });
      }

      if(eventData["driver_location"] != null){
        double driverLat = double.parse(eventData["driver_location"]["latitude"].toString());
        double driverLng = double.parse(eventData["driver_location"]["longitude"].toString());
      ...
      if(eventData["status"] != null){
        statusRide = eventData["status"].toString();
      }
      if(statusRide == "accepted")
      {
      ...
      if(statusRide == "ended"){
        if(eventData["fares"] != null){
          int fare = int.parse(eventData["fares"].toString());
          ...
          if(res == "close")
          {
            if(eventData["driver_id"] != null){
              driverId = eventData["driver_id"].toString();
            }
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(width: 2.0, color: Colors.grey[300]!), // Use Colors.grey[300]!
                        ),
      ...
      if (map != null) {
        ...
        switch (callBack) {
          case Geofire.onKeyEntered:
            NearbyAvailableDrivers nearbyAvailableDrivers = NearbyAvailableDrivers(key: '', latitude: 0.0, longitude: 0.0);
  ...
  void notifyDriver(NearbyAvailableDrivers driver)
  {
    ...
    driversRef.child(driver.key).child("token").once().then((DataSnapshot snap){
      if(snap.value != null)
      {
        ...
        AssistantMethods.sendNotificationToDriver(token, context, rideRequestRef.key ?? "Default value");
      }
      else
      {
        return;
